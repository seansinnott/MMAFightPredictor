...
class Algo1GA extends BaseAlgo1GA {
	...
	public int calculateWinner(char actualWinner, int reachFighterA, int reachFighterB, int weightFighterA, int weightFighterB, int heightFighterA, int heightFighterB) {
...
class Algo1Test {	
	public int testOnUnseenFights(int [] values) {
		...
		Algo1GA algo = new Algo1GA(values);
		int myFitness = 0;
		...
		try {
			for (int i = 0; i < listOfFiles.length; i++) {
				...
				myFitness += algo.calculateWinner(charArray[18],intArray[0],intArray[1],intArray[2],intArray[3],intArray[4],intArray[5]);
	...
	public static void performMutation (Individual indiv) {
		...
		for (int i = 0; i < indiv.getChromosomeLength(); i++) {
			if (probabilityOfMutation >= Math.random()) {
				int mutatedGene = (int) (Math.round (Math.random() * 15));
...
class Individual {
	private int lengthOfChromosome = 3;
	...
	public void createIndividual() {		
		for (int i = 0; i < lengthOfChromosome; i++) {
			int gene = (int) (Math.round (Math.random() * 15));
			genotype[i] = gene;
		}
	}
...
class CalculateFitness {
	...
	public static int getFitness(Individual indiv) {				
		Algo1GA algo = new Algo1GA(indiv.getGenotype());
...
class GeneticAlgorithm1 {
	public static void main (String [] args) {
		...
		while (!maximumFitnessFound && !convergence) {
			...
			System.out.print("[" + population.getFittestInPopulation().getGene(0) + "," + population.getFittestInPopulation().getGene(1) + "," + population.getFittestInPopulation().getGene(2) + "]");
