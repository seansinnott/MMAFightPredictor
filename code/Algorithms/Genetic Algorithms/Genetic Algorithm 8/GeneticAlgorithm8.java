...
class Algo8GA extends BaseAlgo8GA {
...
class Algo8Test {
	public int testOnUnseenFights(int [] values) {
		...
		Algo8GA algo = new Algo8GA(values);
		int myFitness = 0;
...
class Chromosome {	
	...
	public static Individual performCrossover (Individual individual1, Individual individual2) {
		...
		for (int i = 0; i < individual1.getChromosomeLength(); i++) {
			if (probabilityOfCrossover >= Math.random()) {		// probability of 0.8 of crossover being performed at that gene located
...
class CalculateFitness {
	...
	public static int getFitness(Individual indiv) {		
		Algo8GA algo = new Algo8GA(indiv.getGenotype());
