...
class BaseAlgo7GA {	
	// The 'totals' array contains fighter totals (50 and 50 at the start) and each method takes these values and adds and subtracts
...
class Algo7GA extends BaseAlgo7GA {
	...
	public Algo7GA (int [] values) {
		this.values = values;
	}

	// parameters are received from text file in the Genetic Algorithm class.
	// parameters are used to adjust fighter totals appropriately
...
class Algo7Test {
	public int testOnUnseenFights(int [] values) {
		...
		Algo7GA algo = new Algo7GA(values);
		int myFitness = 0;
	...
	public static void performMutation (Individual indiv) {
		...
		for (int i = 0; i < indiv.getChromosomeLength(); i++) {
			if (probabilityOfMutation >= Math.random()) {
				int mutatedGene = (int) (Math.round (Math.random() * 10));
...
class Individual {
	private int lengthOfChromosome = 7;
...
class Population {
	...
	// places an individual into population at a specific position
...
class CalculateFitness {
	...
	public static int getFitness(Individual indiv) {		
		Algo7GA algo = new Algo7GA(indiv.getGenotype());
...
class GeneticAlgorithm7 {
	public static void main (String [] args) {
		...
		while (!maximumFitnessFound && !convergence) {
			...
			// after 40 generation unlikely to see any improvement in fitness
			...
			Algo7Test tst = new Algo7Test();
			int fitnessOnUnseenFights = tst.testOnUnseenFights( population.getFittestInPopulation().getGenotype() );
			
			// print out all relevant data and loop again by creating a new population from fittest idividuals in old population 
			// unless convergence occurs or a solution (predicted every fight perfectly) has been found
			System.out.print("Generation: " + numberOfGenerations + '\t' + "Fittest Individual: " + fittestInCurrentGeneration + '\t' + "Genes: ");
			System.out.print("[" + population.getFittestInPopulation().getGene(0) + "," + population.getFittestInPopulation().getGene(1) + "," + population.getFittestInPopulation().getGene(2) + "," + population.getFittestInPopulation().getGene(3) + "," + population.getFittestInPopulation().getGene(4)  + "," + population.getFittestInPopulation().getGene(5) + "," + population.getFittestInPopulation().getGene(6) + "]");
