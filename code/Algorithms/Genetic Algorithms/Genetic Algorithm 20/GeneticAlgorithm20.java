...
class BaseAlgo20GA {	
	...
	public int [] calcStriking(char bStrikingFighterA, char bStrikingFighterB, char mbStrikingFighterA, char mbStrikingFighterB, int [] totals, int [] values) {
	...
	public int [] calcJiuJitsu(char bJiuJitsuFighterA, char bJiuJitsuFighterB, char mbJiuJitsuFighterA, char mbJiuJitsuFighterB, int [] totals, int [] values) {
	...
	public int [] calcWrestling(char bWrestlingFighterA, char bWrestlingFighterB, char mbWrestlingFighterA, char mbWrestlingFighterB, int [] totals, int [] values) {
	...
	public int [] calcFightingAtHome(char atHomeFighterA, char atHomeFighterB, int [] totals, int [] values) {
		if (atHomeFighterA == 'y') {
			totals[0] += values[12];;
			totals[1] -= values[12];;
		}
...
class Algo20GA extends BaseAlgo20GA {
	...
	public int calculateWinner(char actualWinner, int reachFighterA, int reachFighterB, int weightFighterA, int weightFighterB,
		...
		totals = calcStriking(bStrikingFighterA, bStrikingFighterB, mbStrikingFighterA, mbStrikingFighterB, totals, values);
		totals = calcJiuJitsu(bJiuJitsuFighterA, bJiuJitsuFighterB, mbJiuJitsuFighterA, mbJiuJitsuFighterB, totals, values);
		totals = calcWrestling(bWrestlingFighterA, bWrestlingFighterB, mbWrestlingFighterA, mbWrestlingFighterB, totals, values);
		...
		totals = calcFightingAtHome(atHomeFighterA, atHomeFighterB, totals, values);
...
class Algo20Test {
	public int testOnUnseenFights(int [] values) {
		...
		Algo20GA algo = new Algo20GA(values);
		int myFitness = 0;
	...
	public static void performMutation (Individual indiv) {
		...
		for (int i = 0; i < indiv.getChromosomeLength(); i++) {
			if (probabilityOfMutation >= Math.random()) {
				int mutatedGene = (int) (Math.round (Math.random() * 8));
...
class Individual {
	private int lengthOfChromosome = 13;
	...
	public void createIndividual() {		
		for (int i = 0; i < lengthOfChromosome; i++) {
			int gene = (int) (Math.round (Math.random() * 8));
			genotype[i] = gene;
		}
	}
...
class CalculateFitness {
	...
	public static int getFitness(Individual indiv) {		
		Algo20GA algo = new Algo20GA(indiv.getGenotype());
...
class GeneticAlgorithm20 {
	public static void main (String [] args) {
		...
		// don't stop looping if max fitness is found so as to see if any changes in genotypes lead to
		// improvements in the predicting of unseen fights
		while (!convergence) {
			...
			if (numOfGenerationsWithNoImprovements == 20) {
				convergence = true;
			}			
			...
			Algo20Test tst = new Algo20Test();
			int fitnessOnUnseenFights = tst.testOnUnseenFights( population.getFittestInPopulation().getGenotype() );
			
			// print out all relevant data and loop again by creating a new population from fittest idividuals in old population unless convergence occurs
			System.out.print("Generation: " + numberOfGenerations + '\t' + "Fittest Individual: " + fittestInCurrentGeneration + '\t' + "Genes: ");
			System.out.print("[" + population.getFittestInPopulation().getGene(0) + "," + population.getFittestInPopulation().getGene(1) + "," + population.getFittestInPopulation().getGene(2) + "," + population.getFittestInPopulation().getGene(3) + "," + population.getFittestInPopulation().getGene(4) + "," + population.getFittestInPopulation().getGene(5) + "," + population.getFittestInPopulation().getGene(6) + "," + population.getFittestInPopulation().getGene(7) + "," + population.getFittestInPopulation().getGene(8) + "," + population.getFittestInPopulation().getGene(9) + "," + population.getFittestInPopulation().getGene(10) + "," + population.getFittestInPopulation().getGene(11) + "," + population.getFittestInPopulation().getGene(12) + "]");
