...
class Algo2GA extends BaseAlgo2GA {
...
class Algo2Test {
	public int testOnUnseenFights(int [] values) {
		...
		Algo2GA algo = new Algo2GA(values);
		int myFitness = 0;
...
class Individual {
	private int lengthOfChromosome = 4;
...
class CalculateFitness {
	...
	public static int getFitness(Individual indiv) {		
		Algo2GA algo = new Algo2GA(indiv.getGenotype());
...
class GeneticAlgorithm2 {
	public static void main (String [] args) {
		...
		while (!maximumFitnessFound && !convergence) {
			...
			System.out.print("[" + population.getFittestInPopulation().getGene(0) + "," + population.getFittestInPopulation().getGene(1) + "," + population.getFittestInPopulation().getGene(2) + "," + population.getFittestInPopulation().getGene(3) + "]");
